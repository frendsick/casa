...
| `take_while self:mut$Cursor predicate:fn[char -> bool] -> str` | Consume and return matching text |
...
| `parse_identifier cursor:mut$Cursor -> Result[str ParseError]` | Casa-style identifier |
| `parse_escape cursor:mut$Cursor -> Result[char ParseError]` | Character after a backslash |
| `parse_quoted_string cursor:mut$Cursor -> Result[str ParseError]` | Double-quoted text |
