...
- A reserved equality trait must expose a correctly typed `ne` hook, either with a body or as a required method.
- The standard declaration supplies the negating default so ordinary implementations define only their equality primitive.
- ADR-0082 places both operator hooks on PartialEq and makes Eq inherit their effective shape.
