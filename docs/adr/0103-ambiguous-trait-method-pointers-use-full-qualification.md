...
Selecting a trait implementation method as a first-class function value names both the receiver type and fully instantiated trait when ordinary lookup would be ambiguous:
...
There is no receiver value at the reference site from which to determine the concrete type. Full qualification therefore identifies one trait implementation and one method without expected-type inference.
