...
Casa's standard value containers implement explicit `Clone` when all owned contents can be cloned. `str` and `Bytes` implement `Clone` directly. `array[T N]`, `List[T]`, `Option[T]`, `Result[T E]`, `Map[K V]`, and `Set[T]` provide conditional ordinary trait implementations using the required `Clone` bounds on their owned type parameters.
