...
- Implicit tag-only enum comparison and implicit enum hashing are removed. Enums opt in with `derives`, and generated methods include payload values. `derives Eq` generates PartialEq and Eq; `derives Ord` generates the total comparison primitives and implements PartialEq, Eq, PartialOrd, and Ord, with standard defaults supplying adapters and boolean hooks.
...
- A handwritten trait hook overrides the corresponding derived fallback under ADR-0090; only missing hooks are generated. Two handwritten implementations remain a conflict.
