...
This supersedes ADR-0006's shared writable `.data` semantics. Sharing that changes mutation or destruction behavior conflicts with affine ownership. String literals are copied `str` views of read-only static storage under ADR-0023. Converting a literal to an owned `String` copies its bytes, so each owner remains independent.
