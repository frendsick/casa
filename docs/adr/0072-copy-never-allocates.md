...
Types that own indirect storage, including `array[T]`, `str`, `Bytes`, dynamic collections, and compiler-indirected recursive values, are never `Copy`. Allocation does not make raw duplication of their owning handles safe.
