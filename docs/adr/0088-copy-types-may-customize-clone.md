...
A type conforming to standard Copy may provide its own ordinary Clone implementation. Copy conformance supplies fieldwise Clone behavior only as a fallback when no explicit implementation exists; an explicit implementation takes precedence and is not a duplicate-conformance error.
...
- `derives Copy` plus a handwritten Clone conformance is valid; the handwritten method replaces the fieldwise fallback.
