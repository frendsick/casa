...
- PartialEq provides the shared `eq` and `ne` operator hooks, while Eq is the explicit total-equality refinement. PartialOrd provides `partial_cmp` plus the four boolean ordering hooks; Ord adds `cmp` and total-order conformance.
