...
Generic declarations express representation-safe duplication with the ordinary bound syntax `[T: Copy]`. Copy is a methodless standard-library marker trait with compiler-validated conformances. Built-in conformance is automatic, and user-defined structs and enums request it with `derives Copy` or an equivalent validated empty conformance. The intrinsic stack effects of `dup` and `over` require the canonical Copy trait on the value they duplicate.
...
- Trusting an unchecked marker conformance would let a type claim Copy without proving that raw duplication preserves ownership.
...
- Concrete eligibility remains recursive and compiler-checked according to the affine ownership model; an eligible user-defined type remains move-only unless it declares Copy conformance.
- Copy has a visible methodless trait declaration and uses ordinary explicit conformance syntax, while the compiler validates the representation-specific safety invariant.
