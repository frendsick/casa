...
Method lookup first considers the exact value type. If that type has no applicable method, a borrowed value may call a method on its referent when the declared receiver permits the access. Type qualification selects the referent method explicitly.
