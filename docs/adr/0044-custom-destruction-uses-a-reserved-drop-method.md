...
A type defines custom cleanup with the reserved inherent method `fn drop mut$self`. This is a compiler lifecycle hook rather than trait conformance. Casa does not define a public `Drop` trait, a derivation, or a generic destructor bound.
...
- A compiler-known `Drop` trait fits explicit conformance syntax, but remains special in invocation and ownership while offering little useful generic abstraction.
